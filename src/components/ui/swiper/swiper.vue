...
export default {
  ...
  data() {
    return {
      ...
      screenWidth: document.body.clientWidth + document.body.clientHeight  // 这里是给到了一个默认值 （这个很重要）
    };
  },
  mounted() {
    const that = this;
    window.onresize = () => {
      return (() => {
        that.screenWidth = document.body.clientWidth + document.body.clientHeight;
      })();
    };
  ...
  methods: {
    ...
    refresh() {
      ...
      this.$nextTick(function() {
        var swiperWrapEl = this.$refs.swiperWrap;
        var length = swiperWrapEl.children.length;
        swiperWrapEl.removeChild(swiperWrapEl.children[0]);
        swiperWrapEl.removeChild(swiperWrapEl.children[length - 2]);
        this.slideEls = [].map.call(this.$refs.swiperWrap.children, el => el);
        if (this.loop) {
          this.$nextTick(function() {
            this._createLoop();
...
<style lang="less" src="./swiper.less" scoped></style>
